<template> <!--BT-->
  <div class="BT" v-on:scroll.passive="onScroll($event)"
       v-infinite-scroll="getGameBT"
       infinite-scroll-disabled="loading"
       infinite-scroll-distance="3">
    <i class="icon_search" @click="$router.push({path:'/class_ify/'+1+'/'+topgame})" v-show="searchIcon"></i>
    <!--搜索框-->
    <div class="search_box">
      <!--<div class="soso_bg" @click="$router.push({path:'/search/'+1});">-->
      <div class="soso_bg" @click="$router.push({path:'/class_ify/'+1+'/'+topgame});">
        <i class="icon_sousuo"></i>
        <p class="soso_input color_999999">{{ topgame }}</p>
      </div>
    </div>
    <!--活动菜单-->
    <Banner :banner="banner" :type="1"></Banner>
    <div class="menu border_eee">
      <div class="menu_a" @click="$router.push({path:'/new_game/'+1});">
        <img class="menu_a_img" src="../../../static/img/02_icon_xinyou.png" alt="新游">
        <span class="menu_a_name color_black">新游</span>
      </div>
      <!--<div class="menu_a" @click="$router.push({path:'/activity'});">-->
        <!--<img class="menu_a_img" src="../../../../static/img/05_icon_hd.png" alt="活动">-->
        <!--<span class="menu_a_name color_black">活动</span>-->
      <!--</div>-->
      <div class="menu_a" @click="$router.push({path:'/ranking_list/'+1});">
        <img class="menu_a_img" src="../../../static/img/02_icon_phb.png" alt="排行榜">
        <span class="menu_a_name color_black">排行榜</span>
      </div>
      <div class="menu_a" @click="$router.push({path:'/fullV'});">
        <img class="menu_a_img" src="../../../static/img/01_icon_manv.png" alt="送满V">
        <span class="menu_a_name color_black">送满V</span>
      </div>
      <!--<div class="menu_a" @click="$router.push({path:'/class_ify'});">-->
        <!--<img class="menu_a_img" src="../../../../static/img/04_icon_fenlei.png" alt="分类">-->
        <!--<span class="menu_a_name color_black">分类</span>-->
      <!--</div>-->
      <div class="menu_a" @click="$router.push({path:'/open_list/'+1});">
        <img class="menu_a_img" src="../../../static/img/03_icon_fenlei.png" alt="开服表">
        <span class="menu_a_name color_black">开服表</span>
      </div>
    </div>
    <div class="game_class" @click.stop="$router.push({path:'/class_ify/'+1+'/'+topgame});"><i class="icon_classify"></i><span class="color_blue m_l">游戏分类</span></div>
    <div class="recommendList" v-if="finetop.game.length">
      <div class="sy_title">精品手游<span class="small1">•火爆</span></div>
      <div class="re_c border_eee">
        ...
        <div class="re_c_box" :style="{width:re_c_box_width}">
          <div class="re_c_game small2" v-for="v in finetop.game" @click.stop="router(v.id, 1, myType.channel)">
            <img v-lazy="myType.domainName + v.logo" alt="" class="re_c_game_logo">
            <p class="re_c_game_name color_black" v-text="v.gamename"></p>
            <p class="color_888888 small2">{{ v.types }}</p>
            <p class="re_c_game_weal" v-if="v.finetopstr" v-text="v.finetopstr">50万金币</p>
          </div>
        </div>
      </div>
      <div v-if="finetop.slide">
        <img class="sy_slide" v-if="finetop.slide.gid != 0" v-lazy="myType.domainName + finetop.slide.pic" @click.stop="router(finetop.slide.gid, 1, myType.channel)">
    ...
    <div v-if="newtop.game.length">
      ...
      <List :list="newtop.game" :type="1"></List>
      <div v-if="newtop.slide">
        <img class="sy_slide" v-if="newtop.slide.gid != 0" v-lazy="myType.domainName + newtop.slide.pic" @click.stop="router(newtop.slide.gid, 1, myType.channel)"></img>
    ...
    <div v-if="weektop.game.length">
      ...
      <List :list="weektop.game" :type="1"></List>
      <div v-if="weektop.slide">
        <img class="sy_slide" v-if="weektop.slide.gid != 0" v-lazy="myType.domainName + weektop.slide.pic" @click.stop="router(weektop.slide.gid, 1, myType.channel)"></img>
...
//        this.$router.push({path:'/game_info/'+id+'/'+1})
