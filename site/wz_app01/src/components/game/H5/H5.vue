<template> <!--BT-->
  <div class="BT" v-on:scroll.passive="onScroll($event)"
    ...
    <div class="search_box">
      <div class="soso_bg" @click="$router.push({path:'/class_ify/'+3+'/'+topgame});">
        <i class="icon_sousuo"></i>
        <p class="soso_input color_999999">{{ topgame }}</p>
      </div>
    </div>
    <!--活动菜单-->
    <Banner :banner="banner" :type="3"></Banner>
    <div class="menu border_eee">
      <div class="menu_a" @click="$router.push({path:'/new_game/'+3});">
      ...
      <!--</div>-->
      <div class="menu_a" @click="$router.push({path:'/ranking_list/'+3});">
        <img class="menu_a_img" src="../../../static/img/02_icon_phb.png" alt="排行榜">
        <span class="menu_a_name color_black">排行榜</span>
      </div>
      <div class="menu_a" @click="$router.push({path:''});">
        <img class="menu_a_img" src="../../../static/img/01_icon_manv.png" alt="赚金币">
        <span class="menu_a_name color_black">赚金币</span>
      </div>
      <div class="menu_a" @click="$router.push({path:'/open_list/'+3});">
        <img class="menu_a_img" src="../../../static/img/03_icon_fenlei.png" alt="开服表">
        <span class="menu_a_name color_black">开服表</span>
      </div>
    </div>
    <div class="game_class" @click.stop="$router.push({path:'/class_ify/'+3+'/'+topgame});"><i class="icon_classify"></i><span class="color_blue m_l">游戏分类</span></div>
    <div class="recommendList" v-if="finetop.game.length">
      ...
      <div class="re_c border_eee">
        ...
        <div class="re_c_box" :style="{width:re_c_box_width}">
          <div class="re_c_game small2" v-for="v in finetop.game" @click.stop="router(v.id, 3, myType.channel)">
      ...
      <div v-if="finetop.slide">
        <img class="sy_slide" v-if="finetop.slide.gid != 0" v-lazy="myType.domainName + finetop.slide.pic" @click.stop="router(finetop.slide.gid, 3, myType.channel)">
    ...
    <div v-if="newtop.game.length">
      ...
      <List :list="newtop.game" :type="3"></List>
      <div v-if="newtop.slide">
        <img class="sy_slide" v-if="newtop.slide.gid != 0" v-lazy="myType.domainName + newtop.slide.pic" @click.stop="router(newtop.slide.gid, 3, myType.channel)"></img>
    ...
    <div v-if="weektop.game.length">
      ...
      <List :list="weektop.game" :type="3"></List>
      <div v-if="weektop.slide">
        <img class="sy_slide" v-if="weektop.slide.gid != 0" v-lazy="myType.domainName + weektop.slide.pic" @click.stop="router(weektop.slide.gid, 3, myType.channel)"></img>
...
//        this.$router.push({path:'/game_info/'+id+'/'+3})
