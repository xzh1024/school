      ...
      <form action="" method="post">
        <ul class="login_form">
          <li class="input">
            ...
            <input class="input_center" type="text" placeholder="请输入要解绑的手机号" v-model="mobile" ref="mobile" v-focus/>
          ...
          <li>
            <input class="input_btn m_t_1rem" type="button" value="立即解绑" @click="submit"/>
          </li>
        </ul>
      </form>
    </div>
...
//      this.getUser();
    ...
    methods: {
      ...
      submit(){
        ...
        //解绑手机号
        this.$http.post('/api/index.php?g=api&m=user&a=unbind_mobile',myUrl)
          .then(response => {
      ...
      getCode(){
        ...
        if(this.codeFlag) {
          ...
          this.$http.get('/api/index.php?g=api&m=userbox&a=send_message', {
            params:{
              mobile: this.mobile,
              type: 4,  //1 注册 2 找回密码 3绑定手机 4 解绑手机
              sign: mySign
            }
          })
            .then(response => {
