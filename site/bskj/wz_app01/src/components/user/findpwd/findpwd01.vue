    ...
    <div class="enroll_main">
      <form action="" method="post">
        <ul class="login_form">
          ...
          <li>
            <input class="input_btn m_t_1rem" type="button" value="下一步" @click="submit"/>
          </li>
        </ul>
      </form>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      submit(){
        ...
        //发起请求
        this.$http.post('/api/index.php?g=api&m=userbox&a=check_smscode',myUrl)
          .then(response => {
      ...
      getCode(){
        ...
        if(this.codeFlag) {
          ...
          this.$http.get('/api/index.php?g=api&m=userbox&a=send_message', {
            params:{
              mobile: this.mobile,
              type: 2,  //1 注册 2 找回密码 3绑定手机 4 解绑手机
              sign: mySign
            }
          })
            .then(response => {
