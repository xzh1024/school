...
function Index() {
  ...
  const { downloadCenterShow } = comprehensiveQuery;
  ...
  const handleSearch = value => {
    ...
    comprehensiveQuery.searchParams = value;
    comprehensiveQuery.pageCount = 1;
    comprehensiveQuery.getList(value);
    return value;
  };
  ...
  return (
    <>
            ...
            <>
              ...
              <Col span={6} order={13 + !showMoreSearch}>
                <Form.Item label="导入ERP日期" name="importErpDate">
                  <RangePicker disabledDate={currentDate => dayjs(currentDate).isAfter(dayjs())} allowClear className='w-full'/>
                </Form.Item>
              </Col>
      ...
      {downloadCenterShow && (
        <FilesDownload
          onCancel={() => runInAction(() => (comprehensiveQuery.downloadCenterShow = false))}
        ></FilesDownload>
      )}
    </>
  );
