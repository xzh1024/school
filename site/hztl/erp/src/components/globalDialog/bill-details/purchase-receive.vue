...
import { PURCHASE_RECEIVE_DETAILS } from "./constants";
...
export default {
  ...
  data() {
    return {
      keyScope: Symbol(PURCHASE_RECEIVE_DETAILS.name),
      baseDetailsColumns: PURCHASE_RECEIVE_DETAILS.columns.filter(
        item => item.type !== "controls"
      ),
      tableName: PURCHASE_RECEIVE_DETAILS.name,
      loading: false,
      header: {},
      details: [],
      dateFormat,
  ...
  computed: {
    ...
    statusTxt() {
      return PURCHASE_RECEIVE_STATUS[this.header.status];
    }
  },
  watch: {
    visible: {
