      ...
      >
        ...
        <div class="container-box">
          <div v-show="header.createdByName">
            制单人：
            <span class="text-font">{{ header.createdByName }}</span>
          </div>
        </div>
      </div>
    </div>
...
<script>
import { reqPurchaseDispatchArrivalDetail } from "@/api/purchase/purchaseDispatch";
...
import { PURCHASE_URGENT_DETAILS } from "./constants";
...
export default {
  ...
  data() {
    return {
      keyScope: Symbol(PURCHASE_URGENT_DETAILS.name),
      baseDetailsColumns: PURCHASE_URGENT_DETAILS.columns.filter(
  ...
  computed: {
    ...
    statusTxt() {
      return PURCHASE_URGENT_STATUS[this.header.status];
    }
  },
  watch: {
    visible: {
  ...
  methods: {
    loadData() {
      ...
      this.loading = true;
      reqPurchaseDispatchArrivalDetail(this.billId)
        .then(data => {
          this.loading = false;
