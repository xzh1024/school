...
import { DETAIL_PURCHASE_RETURN } from "./constants";
...
export default {
  ...
  data() {
    return {
      keyScope: Symbol(DETAIL_PURCHASE_RETURN.name),
      baseDetailsColumns: DETAIL_PURCHASE_RETURN.columns.filter(
        item => item.type !== "controls"
      ),
      tableName: DETAIL_PURCHASE_RETURN.name,
      loading: false,
      header: {},
      details: [],
      dateFormat,
  ...
  computed: {
    ...
    statusTxt() {
      return PURCHASE_RETURN_STATUS[this.header.status];
    }
  },
  watch: {
    visible: {
  ...
  methods: {
    loadData() {
      ...
      this.loading = true;
      reqReturnBillingInit(this.billId)
        .then(data => {
          this.loading = false;
