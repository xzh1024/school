...
import { DETAIL_STORAGE_OUT } from "./constants";
import { OUT_BILL_ENUM } from "./constants/storage/enum";
import BillPanel from "@/components/bill-panel/BillPanel";
import * as store from "@/utils/store";

let oldKeyScope;
export default {
  name: "StorageOutDialog",
  ...
  data() {
    return {
      keyScope: Symbol(DETAIL_STORAGE_OUT.name),
      tableName: DETAIL_STORAGE_OUT.name,
      loading: false,
      header: {},
      details: [],
      dateFormat,
      formateAddress,
      billEnum: OUT_BILL_ENUM
    };
  },
  computed: {
    baseDetailsColumns() {
      const baseDetailsColumns = [...DETAIL_STORAGE_OUT.columns];
