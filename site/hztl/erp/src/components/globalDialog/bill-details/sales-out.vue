...
<script>
import { salesOutBillDetails } from "@/api/sales/salesOut";
...
import { DETAIL_SALES_OUT } from "./constants";
...
export default {
  ...
  data() {
    return {
      keyScope: Symbol(DETAIL_SALES_OUT.name),
      tableName: DETAIL_SALES_OUT.name,
      loading: false,
      header: {},
      details: [],
      dateFormat,
  ...
  computed: {
    baseDetailsColumns() {
      const baseDetailsColumns = [...DETAIL_SALES_OUT.columns];
      baseDetailsColumns.splice(18, 0, ...getExtraColumns(this.mode));
  ...
  methods: {
    loadData() {
      ...
      this.loading = true;
      salesOutBillDetails(this.billId)
        .then(data => {
          this.loading = false;
