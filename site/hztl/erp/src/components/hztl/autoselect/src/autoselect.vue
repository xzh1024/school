...
import HtAutocomplete from "../../autocomplete";
  ...
  computed: {
    ...
    useValue: {
      get() {
        ...
        const obj = this.useOptions.find(item => {
        ...
        let disValue = this.filterZero && this.value === 0 ? null : this.value;

        if (![undefined, null].includes(this.defaultValue)) {
          // 优先显示设置的默认值
          disValue = this.defaultValue;
        }
      ...
      set(name) {
        ...
        if (this.value !== value || this.useDefaultValue !== useDefaultValue) {
          this.$emit(
            "update:defaultValue",
            useDefaultValue || "",
