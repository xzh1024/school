...
import { getBillsPurchaseBySingle } from "@/api/base/search";
...
export default {
  ...
  data() {
    ...
    return {
      loading: false,
      warningMsg: "",
      tableName: "PurchaseBillingEditDetailBoxPartInofList",
      tableColumns: [
  ...
  watch: {
    ...
    isCollapse() {
      cacheIsCollapse(this.isCollapse, "PurchaseBillingBasketBilling");
    }
  },
  mounted() {
    this.isCollapse = loadIsCollapse(
      this.isCollapse,
      "PurchaseBillingBasketBilling"
    );
  },
  methods: {
    setAllMarkUpRate() {
    ...
    setMarkUpRate(value, key) {
      this.salePricesMarkUpRate[key] = this.getSaleProfit(value);
    },
    getSaleProfit(price) {
      if (Number(this.ruleForm.taxedPrice) && Number(price)) {
        return parseFloat(
          Number(
            ((Number(price || 0) - Number(this.ruleForm.taxedPrice)) /
    ...
    resetSalePrice(value, key) {
      if (Number(this.ruleForm.taxedPrice) && Number(value)) {
        this.ruleForm.salePrices[key] = Number(
          Number(
            (value / 100) * Number(this.ruleForm.taxedPrice) +
              Number(this.ruleForm.taxedPrice)
          ).toPrecision(12)
        ).toFixed(2);
      } else {
    ...
    setSalePrices(price) {
      if (price || [0, "0"].includes(price)) {
        reqPurchasePriceSettings({
          ...
          companyId: this.header.companyId,
          partId: this.partData.partId,
          ...
          this.ruleForm = Object.assign(this.ruleForm, {
            priorityCategory:
              res.data?.priorityCategory ?? this.priorityCategory,
          ...
          const option = this.priorityCategorys.find(
            item => res.data?.priorityCategory == item.id
          );
          if (option) {
            this.displayPriorityCategory = `${defaultPriorityCategory}（${option?.name}）`;
          }
  ...
  render() {
    return (
      <ht-dialog
        ...
        width="700px"
        top="7vh"
        class="dialog-body ignore-top-hotkeys editPurchaseBill"
      >
        <div class="container-box">
          {!this.warningMsg ? (
            <el-row gutter={5} style="height: 535px;">
                ...
                <ht-card title="采购信息" style="height: 100%;">
                  <div slot="after-title" style="text-align: right;">
                    <ht-select
                      ...
                      default-value={this.defaultPriorityType}
