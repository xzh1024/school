...
import { cancelPurchaseBillParts } from "@/api/purchase/purchaseList";
...
export default {
  ...
  data() {
    return {
      ...
      totalTableData: [],
      tableName: `${PURCHASE_BILLING_DETAILS.name}CancelBox`,
      tableData: [],
      selectedRows: [],
      total: 0,
      page: 1,
            ...
            >
              <el-form-item prop="curTermQty">
                <HtFormatNumber
                  ...
                  min={0}
                  max={Number(row.waitPurchaseQty || 0)}
                  precision-type="qty"
  ...
  methods: {
    ...
    cancelAllParstHandle(e) {
      ...
      const totalData = this.totalData.map(item => ({
        ...item,
        curTermQty: Number(item.waitPurchaseQty || 0)
      }));
    ...
    cancelParstHandle(selectedRows, e) {
      ...
      cancelPurchaseBillParts(this.headerData.id, { details })
        .then(() => {
