...
export type SearchFormData = Pick<
  SearchParams,
  | "relatedCompanyId"
  | "code"
  | "codePattern"
...
const RULES = {
  relatedCompanyId: [
    { required: true, message: "请选择调入分公司", trigger: "change" }
  ]
};
...
export default class SearchPartsBox extends Mixins(SearchMixin) {
  ...
  get defaultTab0Focus() {
    return this.disabledRelatedCompany ? "code" : "relatedCompanyId";
  }
  ...
  onVisibleChange() {
    if (this.visible) {
      ...
      this.handleAllocOutCompanyIdChange(this.formData.relatedCompanyId);
  ...
  private async handleAllocOutCompanyIdChange(relatedCompanyId?: number) {
  ...
  private getWarehouseOptions(relatedCompanyId?: number) {
    return new Promise((resolve: Function) => {
      if (relatedCompanyId) {
        this.searchLoadAllocWarehouses({
          status: 0,
          outCompanyId: Number(this.companyId || 0),
          inCompanyId: Number(relatedCompanyId || 0)
        })
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          .then((res: any) => {
            resolve(res || []);
  ...
  private getDefualtWarehouses(relatedCompanyId?: number) {
    return new Promise((resolve: Function) => {
      if (relatedCompanyId) {
        getStaffSystemParamsDetail({
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          .then((res: any) => {
            if (res.value && res.value[`${TYPE}${relatedCompanyId}`]) {
              resolve(res.value[`${TYPE}${relatedCompanyId}`]);
            } else {
              resolve([]);
            }
          })
    ...
    return (
      ...
      >
        ...
        <el-form
          ...
          show-message={false}
        >
          <el-form-item prop="relatedCompanyId" label="调入分公司">
            {this.isBatch ? (
              <ht-autoselect
                key={this.isBatch}
          ...
          <el-form-item label="所属仓库">
            <div class="flex-container">
              <ht-select
                disabled={!this.formData.relatedCompanyId}
        ...
        <set-default-warehouses-box
          ...
          options={this.warehouseOptions}
          type={`${TYPE}${this.formData.relatedCompanyId}`}
