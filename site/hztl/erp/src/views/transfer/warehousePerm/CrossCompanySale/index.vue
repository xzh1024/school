...
} from "@/api/transfer/warehouseSale";
...
export default {
  ...
  methods: {
    ...
    async initList() {
      this.loading = true;
      const data = await getCompanyList().catch(err => {
        this.$message.error(err);
        this.loading = false;
      });
      const { value, canEdit } = data.allowBranchSale;
