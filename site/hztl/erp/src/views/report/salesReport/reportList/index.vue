    ...
    <ht-card no-padding style="padding:16px;">
      ...
      >
        <el-row :gutter="10">
          ...
          <el-col :span="5">
            <el-form-item label="客户" prop="cooperator">
              <ht-select
...
import {
  REPORT_SALES_LIST_CHILD,
  REPORT_SALES_LIST_CHILDREN,
  REPORT_SALES_LIST_SEARCH
} from "@/views/report/constants";
import { COMPANY, DATE, PEOPLE } from "../../group";
import { BILLS, PARTS, SUPPLIER } from "../group";
import {
...
export default {
  ...
  data() {
    return {
      ...
      tableDataChild: [],
      multipleSelections: [],
      tableName: REPORT_SALES_LIST_SEARCH.name,
      childTableName: REPORT_SALES_LIST_CHILD.name,
      defaultColumns: REPORT_SALES_LIST_SEARCH.columns, //默认父列表维度
      defaultChildColumns: REPORT_SALES_LIST_CHILD.columns, //默认父列表维度
      indexColumns: REPORT_SALES_LIST_CHILDREN.columns, //指标列表
      mainColmns: REPORT_SALES_LIST_CHILDREN.columns.concat(
        REPORT_SALES_LIST_SEARCH.columns
      ), //父列表，默认使用父列表维度列表
      defaultChildCol: REPORT_SALES_LIST_CHILDREN.columns.concat(
        REPORT_SALES_LIST_CHILD.columns
      ), //子列表维度
      expandColumn: {
        prop: "expand",
        label: "操作",
