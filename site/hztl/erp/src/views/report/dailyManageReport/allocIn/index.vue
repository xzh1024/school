<template>
  <div class="compo-item-wrap dailyReport">
    <ht-card v-if="activetab === 1" title="调拨入库">
      <ht-setting-table
    ...
    <span v-if="activetab === 2 && detail.length == 0" class="no-data-tips"
      >[此类目暂无数据]</span
    >
    <ht-card v-if="activetab === 2" title="调拨入库">
      <!-- 分组 -->
      <div
        class="group"
...
<script>
import { allocInSummary } from "@/api/dailyManageReport";
...
export default {
  ...
  methods: {
    getSummary() {
      this.loading = true;
      allocInSummary(this.localQuery)
        .then(res => {
          this.loading = false;
          const {
            Rows,
    ...
    getGroups() {
      this.detail = [];
      allocInSummary(this.localQuery).then(res => {
        const { Rows } = res;
        if (Rows) {
          this.initGroups(res.Rows);
          const groups = [];
