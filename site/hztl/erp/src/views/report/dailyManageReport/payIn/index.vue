<template>
  <div class="compo-item-wrap">
    <ht-card v-if="activetab === 1" title="应收登记">
      <ht-setting-table
    ...
    <span v-if="activetab === 2 && detail.length == 0" class="no-data-tips"
      >[此类目暂无数据]</span
    >
    <ht-card v-if="activetab === 2" title="应收登记">
      <!-- 分组 -->
      <div
        class="group"
...
<script>
import { payInSummary, payInGroup } from "@/api/dailyManageReport";
import { TABLE_COLUMN_PAY_IN_SUMMARY } from "@/constants/report/payIn";
...
export default {
  ...
  computed: {
    columns() {
      return TABLE_COLUMN_PAY_IN_SUMMARY.columns;
    }
  },
  methods: {
    getSummary() {
      this.loading = true;
      payInSummary(this.localQuery)
        .then(res => {
          this.loading = false;
          const {
            rows,
