<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="移仓状态">
        <ht-autoselect
          v-if="setFoucs('status')"
          v-delay-focus="setFoucs('status')"
          v-model="formData.status"
          :options="searchBaseData.allMovingStutas"
          :trigger-on-focus="false"
          placeholder="移仓状态"
          highlight-first-item
        />
      ...
      <el-form-item label="货主">
        <ht-autoselect
          v-if="setFoucs('ownerCompanyId')"
          v-delay-focus="setFoucs('ownerCompanyId')"
  ...
  methods: {
    ...
    handleSearch() {
      ...
      switch (this.activeTab) {
        ...
        case TABS[1].id:
          params = {
            ..._.pick(this.formData, [
              "brands",
              "productionPlaces",
              "property",
              "warehouseId"
            ]),
            codes: this.batchData.map(item => item.code),
            demandQties: this.batchData.map(item => item.num)
          };
          break;
        default:
          break;
