        ...
        <ht-card title="托运信息" style="margin-top: 5px">
          ...
          <el-row :gutter="10">
            ...
            <el-col :span="8">
              ...
              <el-form-item label="发货主题" prop="subject">
                <span
                  v-show="showText"
                  class="text-font-primary text-container"
...
import SrImgUpload from "@/components/base/SrImgUpload"; // 收发货 登记单 上传
...
export default {
  ...
  computed: {
    ...
    showBtns() {
      return !(this.isCheck || this.isCompelte || this.isSettle);
    },
    showText() {
      return (
        this.isConfirm ||
  ...
  methods: {
    ...
    confirm() {
      if (this.isCheck || this.isSettle) return;
      if (this.isSignFor) {
        this.$emit("on-sign-for");
      } else if (this.isConfirm) {
          ...
          if (valid) {
            this.handleSureRigister({
              ...
              otherCharges: this.ruleForm.otherCharges
                ...
                : 0,
              picture: this.imgFileList.length
                ? this.imgFileList[0].origin
                : "",
              qty: this.totalQty,
    ...
    createLogisticsRegister(billInfo, details) {
      reqCreateLogisticsRegister({ billInfo, details }).then(() => {
        this.$message({
          message: "添加登记单成功！",
          type: "success",
          showClose: true
    ...
    updateLogisticsRegister(billInfo) {
      reqUpdateLogisticsRegister({ billInfo }, this.registerId).then(() => {
        this.$message({
          message: "更新登记单成功！",
          type: "success",
          showClose: true
