...
export default {
  ...
  computed: {
    ...
    showTitle() {
      return this.editId !== 0 ? "编辑货区" : "新增货区";
    },
    isAdd() {
  ...
  methods: {
    loadDetail() {
      if (this.editId !== 0) {
        reqGetStoreArea(this.editId).then(res => {
      ...
      if (this.editId !== 0) {
        this.$refs["addFormRef"].validate((valid, err) => {
          if (valid) {
            ...
            reqStoreAreasUpdate(this.addFormData)
              .then(() => {
                this.submitLoading = false;
                this.$message.success({
                  message: "修改货区 " + this.addFormData.name + " 成功"
                });
                this.$emit("reload");
                this.close();
              })
        ...
        if (this.editTypeRadio === "single") {
          ...
          this.$refs["formRef"].validate((valid, err) => {
            if (valid) {
              this.submitLoading = true;
              reqStoreAreasAdd(reqData)
                .then(() => {
                  this.submitLoading = false;
                  this.$message.success({
                    message: "新增货区 " + this.formData.singleAddName + " 成功"
                  });
                  this.$emit("reload");
                  this.close();
                })
