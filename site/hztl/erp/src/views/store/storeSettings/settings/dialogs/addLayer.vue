  ...
  >
    <div style="height: 250px; padding: 20px;" v-loading="submitLoading">
      <el-form
        ...
        style="height: 180px"
      >
        <el-form-item label="货层名称" prop="name">
          <ht-input
...
export default {
  ...
  data() {
    return {
      ...
      editTypeRadio: "batch",
      rules: {
        name: [{ required: true, message: "请输入货层名称", trigger: "blur" }]
      }
    };
  },
  computed: {
    ...
    showTitle() {
      return this.editId !== 0 ? "编辑货层" : "新增货层";
    },
    isAdd() {
  ...
  methods: {
    loadDetail() {
      if (this.editId !== 0) {
        reqGetStoreLayer(this.editId).then(res => {
      ...
      if (this.editId !== 0) {
        this.$refs["addFormRef"].validate((valid, err) => {
          if (valid) {
            ...
            reqStoreLayerUpdate(this.addFormData)
              .then(() => {
                this.submitLoading = false;
                this.$message.success({
                  message: "修改货层 " + this.addFormData.name + " 成功"
                });
                this.$emit("reload");
                this.close();
              })
        ...
        const reqData = {};
        reqData.shelfId = this.warehousesInfo.shelfId;
        if (this.editTypeRadio === "single") {
          reqData.name = this.formData.singleAddName;
          this.$refs["formRef"].validate((valid, err) => {
            if (valid) {
              this.submitLoading = true;
              reqStoreLayerAdd(reqData)
                .then(() => {
                  this.submitLoading = false;
                  this.$message.success({
                    message: "新增货层 " + this.formData.singleAddName + " 成功"
                  });
                  this.$emit("reload");
                  this.close();
                })
