<template>
  <ht-card title="货层" style="margin: 5px 0 0 5px;" class="auto-card">
    ...
    <add-Layer :visible.sync="addShow" @reload="reload" :editId="editId" />
  </ht-card>
</template>

<script>
import addLayer from "@/views/store/storeSettings/settings/dialogs/addLayer";
import { reqStoreLayerDelete, reqStoreLayerList } from "@/api/store/settings";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableColumns: [
        controlsColumn,
        { prop: "name", label: "货层名称", width: 100 }
      ]
    };
  },
  watch: {
    selectedRows() {
      if (
        ...
        this.selectedRows[0].id != this.warehousesInfo.layerId
      ) {
        this.$store.commit(
          "admin/warehouseSettings/changelayerId",
          this.selectedRows[0].id
        );
      }
    },
    "warehousesInfo.shelfId"() {
      if (this.warehousesInfo.shelfId) {
        this.loadData();
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row, e) {
      e.stopPropagation();
      this.$confirm(`确定要删除货层 ${row.name} ？`, "提示", {
        type: "warning"
      })
        .then(() => {
          reqStoreLayerDelete(row.id)
            .then(() => {
              this.$message.success({ message: `成功删除货层 ${row.name}` });
              this.$store.commit("admin/warehouseSettings/changelayerId", 0);
              this.loadData();
            })
            .catch(() => {});
        })
