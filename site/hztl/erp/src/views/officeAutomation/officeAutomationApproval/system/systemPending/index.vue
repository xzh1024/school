...
import ApprovingDialog from "../components/approvingDialog.vue";
import { OfficeAutomationService } from "../../constant/services";
const officeAutomationService = new OfficeAutomationService();

@Component({
  name: "SystemPending",
  components: { ApprovingDialog }
})
export default class SystemPending extends Mixins(PaginationDataMixin) {
        ...
        render: (value: string, row: FeeApplicationModel) => {
          return (
            <el-button
              ...
              size="mini"
              plain
              v-show={row.status === "toAudit"}
              onClick={() => this.showApprovingDialog(row)}
            >
              审批
            </el-button>
          );
  ...
  get approvingDialog(): ApprovingDialog {
    return this.$refs.approvingDialog as ApprovingDialog;
  }

  protected showApprovingDialog(row: FeeApplicationModel) {
    this.approvingDialog.show(row);
  }
