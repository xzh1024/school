...
const defaultParams = {
  repairDispatch: true,
  repairWorkOrderStartEndStep: true,
  repairWorkOrderAmountFloor: true,
  repairCarWashDistribute: true,
  repairHealth: [],
  repairCustomerSituationRemind: []
};
...
export default {
  ...
  methods: {
    loadParams() {
      this.loading = true;
      getSystemParamsDetail({ module: "repair", type: this.paramsType })
        .then(res => {
          /* paramsType==='repairCustomerSituationRemind'是特殊的多选框情况，特殊处理 */
          if (this.paramsType === "repairCustomerSituationRemind") {
            const arr = [];
            ...
            this.params = { ...defaultParams, [this.paramsType]: arr };
          } else {
    ...
    handleBatchSetBoxSure(params) {
      ...
      console.log(this.parametersTableData);
    },
    saveHandle(e) {
      if (e) {
        e.stopPropagation();
      ...
      let defaultCompanyInfo = this.params[this.paramsType];
      /* 对客情提醒 repairCustomerSituationRemind 做特殊处理*/
      if (this.paramsType === "repairCustomerSituationRemind") {
        const defaultCompanyValue = {};
        this.params[this.paramsType].forEach(item => {
          defaultCompanyValue[item] = true;
        });
        defaultCompanyInfo = defaultCompanyValue;
        this.parametersTableData.forEach(company => {
          const repairCustomerSituationRemind = {};
          company.repairCustomerSituationRemind.forEach(item => {
            repairCustomerSituationRemind[item] = true;
          });
          company.repairCustomerSituationRemind = repairCustomerSituationRemind;
        });
      }
      this.loading = true;
      updateSystemParamsDetail({
