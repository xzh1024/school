  ...
  <div>
    <el-radio-group v-model="activeTab" size="mini" @change="handleTypeChange">
      <el-radio label="0" border>
        配件调出类
        <span class="total-numer">({{ totalNumers.allocOutNum }})</span>
      </el-radio>
      <el-radio label="1" border>
        配件调入类
        <span class="total-numer">({{ totalNumers.allocInNum }})</span>
      </el-radio>
    </el-radio-group>
    <ht-table
      :key-scope="keyScope"
...
import { FINANCE_SETTLE_CENTER_TRANSFER } from "@/constants";
...
export default {
  ...
  data() {
    return {
      ...
      tableName: FINANCE_SETTLE_CENTER_TRANSFER.name,
      baseTableColumns: FINANCE_SETTLE_CENTER_TRANSFER.columns,
      tableColumns: []
    };
  },
  computed: {
    ...
    loadData() {
      ...
      return loadBillSettle({
        ...
        .then(res => {
          this.loading = false;
          this.summaries = {
            amount: res.settlementGroups["调拨挂账"][this.activeTab].amount
          };
          this.tableData = res.rows
            ? res.rows.map(item => ({
                ...item,
                enhancedSettlementType:
                  item.enhancedSettlementType == "调拨挂账"
                    ? "内部挂账"
                    : item.enhancedSettlementType,
                settlementType:
                  item.settleType == "调拨挂账" ? "内部挂账" : item.settleType
              }))
            : [];
