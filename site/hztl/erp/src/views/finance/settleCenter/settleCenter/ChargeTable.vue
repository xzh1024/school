  ...
  <div>
    <el-radio-group v-model="activeTab" size="mini" @change="handleTypeChange">
      <el-radio label="0" border>
        配件销售类
        <span class="total-numer">({{ totalNumers.ontickSaleNum }})</span>
      </el-radio>
      <el-radio label="1" border>
        配件采购类
        <span class="total-numer">({{ totalNumers.ontickPurcNum }})</span>
      </el-radio>
      <el-radio label="2" border v-if="showYx">
        维修类
        <span class="total-numer">({{ totalNumers.ontickRepaireNum }})</span>
      </el-radio>
    </el-radio-group>
    <ht-table
      :key-scope="keyScope"
...
import { FINANCE_SETTLE_CENTER_CHARGE } from "@/constants";
...
export default {
  ...
  data() {
    return {
      ...
      tableName: FINANCE_SETTLE_CENTER_CHARGE.name,
      baseTableColumns: FINANCE_SETTLE_CENTER_CHARGE.columns,
      tableColumns: []
    };
  },
  computed: {
    ...
    loadData() {
      ...
      return loadBillSettle({
        ...
        .then(res => {
          this.loading = false;
          this.summaries = {
            amount: res.settlementGroups["挂账"][this.activeTab].amount
          };
