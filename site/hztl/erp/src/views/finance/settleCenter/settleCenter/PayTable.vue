  ...
  <div>
    <el-radio-group v-model="activeTab" size="mini" @change="handleTypeChange">
      <el-radio label="0" border>
        待收款
        <span class="total-numer">({{ totalNumers.nowpayInNum }})</span>
      </el-radio>
      <el-radio label="1" border>
        待付款
        <span class="total-numer">({{ totalNumers.nowpayOutNum }})</span>
      </el-radio>
    </el-radio-group>
    <ht-table
      v-loading="loading"
...
import { FINANCE_SETTLE_CENTER_NOWPAY } from "@/constants";
...
export default {
  ...
  data() {
    return {
      ...
      tableName: FINANCE_SETTLE_CENTER_NOWPAY.name,
      baseTableColumns: FINANCE_SETTLE_CENTER_NOWPAY.columns,
      tableColumns: []
    };
  },
  computed: {
  ...
  watch: {
    ...
    paymentType: {
      handler(value) {
        ...
        this.$emit("paymentTypeChange", value);
      },
      immediate: true
    },
    selectedRows(value) {
  ...
  methods: {
    ...
    handleTypeChange() {
      // this.tableColumnsChange(this.baseTableColumns);
      this.changeTab();
      this.currentPage = 1;
      this.loadData();
    },
    ...
    loadData() {
      ...
      return loadBillSettle({
        ...
        .then(res => {
          this.loading = false;
          this.summaries = {
            amount: res.settlementGroups["现付"][this.activeTab].amount
          };
