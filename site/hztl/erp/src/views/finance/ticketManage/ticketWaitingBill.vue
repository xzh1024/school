<template>
  ...
  <div class="container-box" style="height:100%">
      ...
      <div class="flex-container column-flex">
        <div class="flex-block-bg btns">
          <el-button
            type="primary"
            size="small"
            v-if="hasBelongsPermWithoutRoute('create', 'financials.invoiceIn')"
    ...
    <export-table
      ...
      table-name="ticket-waiting-bill-export"
...
import { ExportTable } from "@/components/table"; // 导出
...
import {
  invoicesInWaitingList,
  invoicesInWaitingExport
  ...
  methods: {
    ...
    resList(searchData, pagination) {
      ...
      searchData.endDate = this.otherRange
        ? this.otherRange[1]
          ...
          : null
        : null;
      invoicesInWaitingList({ ...searchData, ...pagination })
        .then(res => {
          this.totalData = res || {};
    ...
    openDialog() {
      ...
      if (selectIds.length > 1) {
        this.$message.error("系统不支持多个往来单位同时收票，请重新勾选!");
        return false;
      } else {
    ...
    statusHandle(row) {
      this.$router.push({
        path: "/ticketManage",
        query: { id: "ticketBill", billNo: row.billNo }
      });
    },
    ...
    outExoportHandle(sampleColums) {
      ...
      data.endDate = this.otherRange
        ? this.otherRange[1]
          ...
          : null
        : null;
      return invoicesInWaitingExport({
        ...data,
