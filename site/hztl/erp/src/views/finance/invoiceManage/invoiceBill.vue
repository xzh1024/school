<template>
  ...
  <div class="container-box" style="height:100%">
          ...
          >
            <el-row :gutter="10">
              <el-col :span="6">
                ...
                <el-form-item label="开票分公司" label-width="80px">
                  <ht-select
                    multiple
                    collapse-tags
                    filterable
    ...
    <export-table
      ...
      table-name="invoice-bill-export"
...
export default {
  ...
  methods: {
    ...
    resList(searchData, pagination) {
      ...
      searchData.endDate = this.otherRange
        ? this.otherRange[1]
          ...
          : null
        : null;
      invoicesOutList({ ...searchData, ...pagination })
        .then(res => {
          this.totalData = res || {};
    ...
    invalidHandle(row) {
      console.log(row);
      this.$confirm(`确定要作废开票单[${row.billNo}]吗？`, "作废", {
        type: "warning"
      })
        .then(() => {
          // 作废操作
          invoicesOutvalid({ id: row.id }).then(() => {
            this.resList(this.searchForm, {
              page: this.currentPage,
              pageSize: this.pageSize
            });
    ...
    // 点击单号
    billNoHandle(row) {
      invoicesOutDetail({ billId: row.id }).then(res => {
        this.details = res || {};
        this.details.header = row;
        // 车牌号-车型  本期不支持
        // this.details.header.outExtraInfo = {};
        // this.details.header.outExtraInfo.plateNumber =
        //   row.extraInfo.data.plateNumber || "";
        // this.details.header.outExtraInfo.vehModel =
        //   row.extraInfo.data.vehModel || "";
