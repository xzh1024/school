...
import { COMPANY_SUMMARY_LIST, COMPANY_SUMMARY_SUB } from "./constants";
...
export default class CompanyComponent extends Mixins(
  SearchDetailTable,
  SearchMixin
) {
  ...
  protected get exportName() {
    return `物流代收汇总（按往来单位汇总）-${dateFormat(curDate)}`;
  }
  ...
  protected tableName = COMPANY_SUMMARY_LIST.name;
  protected columns = COMPANY_SUMMARY_LIST.columns;
  protected detailTableName = COMPANY_SUMMARY_SUB.name;
  protected detailColumns = COMPANY_SUMMARY_SUB.columns;

  private formateDetailCloumns() {
    this.detailColumns = COMPANY_SUMMARY_SUB.columns.map(item => {
  ...
  protected showDetail(curData: FinanceSummaryData) {
    const summaryInfo: SummaryInfo = {
      ...
      cooperatorNo: this.curSelected.cooperatorNo,
      cooperatorName: this.curSelected.cooperatorName,
      logisticsNo: curData.cooperatorNo,
      logisticsName: curData.cooperatorName
    };
    this.$emit("show-detail", summaryInfo, {
      ...this.searchParams,
      cooperatorId: this.curSelected.cooperatorId,
      logisticsId: curData.cooperatorId
    });
  }
  private setSortData() {
  ...
  protected renderSearch() {
    return (
      ...
      >
        <el-row gutter={10}>
          <el-col span={6}>
            <el-form-item label="业务公司" label-width="80px">
              <ht-select
          ...
          <el-col span={6}>
            <el-form-item label="开单业务员" label-width="80px">
              <ht-select
