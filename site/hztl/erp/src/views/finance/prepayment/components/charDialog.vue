    ...
    >
      ...
      <el-form-item label="往来单位" prop="cooperatorId">
        <ht-autoselect
          ...
          :disabled="fromDetail"
          :default-value="formData.cooperatorName"
          placeholder="往来单位"
          @change="getBalance"
          remote
          clearable
      ...
      <el-form-item label="可冲金额" prop="commonBalance">
        <ht-input
          v-model="formData.commonBalance"
...
import { chargeSupplement, balance } from "@/api/finance/prevFunds";
...
export default {
  ...
  data() {
    const checkAmount = (_, value, callback) => {
      ...
      if (value > +this.formData.commonBalance) {
        return callback(new Error("不能超过可冲金额"));
      }
      callback();
    };
    return {
  ...
  methods: {
    ...
    getBalance() {
      const { companyId, cooperatorId } = this.formData;
      ...
      balance(query).then(res => {
        if (!res) return;
        const { commonBalance } = res;
        this.formData.commonBalance = +commonBalance;
      });
    },
    ...
    supplement() {
      ...
      chargeSupplement({
        ...this.formData,
        billType: convertObj[this.formData.billType]
      })
        .then(() => {
