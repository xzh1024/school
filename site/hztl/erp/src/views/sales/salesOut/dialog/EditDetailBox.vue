...
import { queryQtyPrice } from "@/api/sales/salesOut";
  ...
  data() {
    const validBillPrice = (rule, value, callback) => {
      if (value || [0, "0"].includes(value)) {
        if (Number(value) < this.ruleForm.preDiscountTaxedPrice) {
          const errTips = "开单价需大于等于售价";
    ...
    const validQtyAndLockedQty = (rule, value, callback) => {
      /* if (
        this.infoData.isCrossCompanySale &&
        Number(value) > Number(this.infoData.dynaQty || 0)
      ) {
        callback(new Error("跨公司销售开单货品数量超过库存数量"));
      } else  */ if (
        Number(value) > 0
      ) {
        callback();
    ...
    return {
      tableName: "SalesOutEditDetailBoxPartInofList",
      tableColumns: [
      ...
      rules: {
        ...
        bookQty: [
          {
            required: true,
            validator: validQtyAndLockedQty,
            trigger: ["change", "blur"]
          }
        ],
        preDiscountTaxedPrice: [
          { validator: validPrice, trigger: ["change", "blur"] }
        ],
        billPrice: [
          { required: true, validator: validBillPrice, trigger: "blur" }
        ],
  ...
  computed: {
    ...
    otherPrices() {
      return getSalePriceOptions({
        retailPrice: this.infoData.salePrices?.retail || "",
        platformPrice: this.infoData.salePrices?.platform || "",
        transferPrice: this.infoData.salePrices?.allot || "",
        wholeSalePrice: this.infoData.salePrices?.wholesale || "",
        wholeSalePrice1: this.infoData.salePrices?.wholesale1 || "",
        wholeSalePrice2: this.infoData.salePrices?.wholesale2 || "",
        wholeSalePrice3: this.infoData.salePrices?.wholesale3 || "",
        wholeSalePrice4: this.infoData.salePrices?.wholesale4 || ""
      });
    },
    localcompanyId() {
  ...
  methods: {
    ...
    async refreshData() {
      ...
      this.saveSrcData = {
        ...
        billPrice: this.ruleForm.billPrice,
    ...
    async getLastData() {
      ...
      const isNegative = this.stockData?.isNegative || false;
      const params = {
        isNegative,
      ...
      switch (this.mode) {
        case 0: {
          ...
          // this.partData.stockIds?.length === 1
          //   ? this.partData.stockIds[0]
          //   : null;
          break;
        }
        case 1: {
      ...
      const res = await queryQtyPrice(params).catch(() => {
    ...
    priceChange(price) {
      ...
      this.ruleForm.billAmount = this.ruleForm.preDiscountTaxedAmount;
      this.setUnTaxedData();
    },
    amountChange(amout) {
      ...
      this.ruleForm.billPrice = this.ruleForm.preDiscountTaxedPrice;
      this.ruleForm.billAmount = amout;
      this.setUnTaxedData();
    },
    ...
    billPriceBlur() {
      if (
        ...
        Number(this.ruleForm.billPrice) < this.ruleForm.preDiscountTaxedPrice
      ) {
        this.ruleForm.billPrice = this.ruleForm.preDiscountTaxedPrice;
        this.ruleForm.billAmount = this.ruleForm.preDiscountTaxedAmount;
      }
    ...
    sureHandle(e) {
      ...
      this.$refs.ruleForm.validate(async (valid, err) => {
        if (valid) {
          //实际数是否小于0
          const hasRelQty = Number(this.infoData.Qty || 0) <= 0;
          //是否负库存销售
          const hasQty = Number(this.infoData.dynaQty || 0) <= 0 ? "预估" : "";
          //有低于成本价销售权限
          const hasPermSellBelow = this.hasBelongsPerm(
            "sellBelowCost",
            this.isEdit ? this.header.belongs : ["self"]
          );
          //非精确到批次
          const hasAvg = this.mode > 0 && !hasQty ? "平均" : "";
          //不是固定价格，客户享受价格，不是负库存，有进货价且 没权限，且 实际数大于0，拦截并提示无权限
          //无进货价不做权限验证
          if (
          ...
          this.$emit("sure", {
            ...
            saleVehModel: this.ruleForm.saleVehModel
              ...
              : [""],
            stockId:
              this.mode === 0 && this.partData.stockId
                ? this.partData.stockId
                : undefined,
            isCrossCompanySale:
              this.partData.ownerCompanyId &&
              Number(this.partData.ownerCompanyId > 0) &&
              this.partData.ownerCompanyId != this.localcompanyId
          });
          //this.cancelHandle();
        } else {
            ...
            <el-col span={10} style="height: 100%;">
              <ht-card title={"销售信息"} style="height: 100%;">
                ...
                >
                  ...
                  <el-form-item prop="billPrice" label="开单价">
                    <HtFormatNumber
