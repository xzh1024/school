    ...
    >
      <el-table-column type="expand" width="26">
        <template slot-scope="props">
          <PackageTableExpand :row="props.row"></PackageTableExpand>
        </template>
      </el-table-column>
      <el-table-column
        type="selection"
...
import PackageSearch from '@/components/goods/project/projectSearch.vue';
import PackageTableExpand from '@/components/goods/package/packageTableExpand.vue';
...
export default {
  ...
  methods: {
    ...
    showEditDialog() {
      ...
      this.packageEditVisible = true;
    },
    hideEditDialog() {
      this.packageEditVisible = false;
    },
    // 查询
    handleQuery() {
    ...
    handleCopy(row) {
      if (this.shareInfo.link) {
        const text = `${this.shareInfo.link}/packageDetail?id=${row.id}&goodsType=${this.formData.type}`;
