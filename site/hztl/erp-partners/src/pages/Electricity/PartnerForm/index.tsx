...
class PartnerForm extends Component<FormProps> {
  componentDidMount() {
    const { dispatch, partsPropertyList, partsPriceList } = this.props;
    ...
    if (partsPropertyList.length === 0) {
      dispatch({
        type: "partnerBaseData/fetchPartsPropertyList",
      });
    }
...
const ElectricityForm = connect(
  ({ partnerBaseData, eleSteps, electricity, loading }: EleConnectState) => ({
    partsPropertyList: partnerBaseData.partsPropertyList,
    partsStoreList: partnerBaseData.partsStoreList,
    partsPriceList: partnerBaseData.partsPriceList,
    partnerList: [
      ...eleSteps.unOpenPartnerList,
      ...electricity.list.map(item => ({ key: item.partnerKey, name: item.partnerName })),
    ],
    saveLoading: loading.effects["eleSteps/createEle"],
  }),
)(PartnerForm);
