    ...
    <div class="search-result">
        <div class="search-header">
          ...
          <div class="search-box">
            <van-dropdown-menu active-color="#FF6E4C" class="search-select">
              <van-dropdown-item
                v-model="searchKey"
...
export default {
  ...
  created(){
    ...
    this.contentList.forEach(p=>{
      let obj = {}
      if(p.content.indexOf('关键')>-1 ){
        obj = {
          value: "keyword",
          text: p.content1,
          content2: p.content2
        }
        this.searchTypeList.push(obj);
      }
      if(p.content.indexOf('OE')>-1){
        obj = {
          value: "oe",
          text: p.content1,
          content2: p.content2
    ...
    this.placeholder = this.searchTypeList[0].content2;
  },
  mounted: function() {
    let _this = this;
    ...
    switch (query.searchType) {
      ...
      case "oe":
        ...
        _this.searchkeyword = query.oe;
        break;
      case "keyword":
        _this.level1.keyWord = query.keyword;
        _this.searchkeyword = query.keyword;
        break;
      case "skuIds":
  ...
  methods: {
    selectChange(val){
      ...
      this.contentList.forEach((p,index)=>{
        if(val == 'oe' && p.content.indexOf("OE")>-1){
            this.placeholder = p.content2;
            this.activeIndex = index;
        }
        if(val == 'keyword' && p.content.indexOf("关键")>-1){
            this.placeholder = p.content2;
            this.activeIndex = index;
        }
      })
    ...
    onSearch() {
      ...
      if (_this.needLoadPageData()) {
        let skuQuery = _this.formDataString(_this.skuListData());
        if(_this.$route.query.couponId) {
          skuQuery += `&couponId=${_this.$route.query.couponId}`
        }
        _this.skuQuery = skuQuery;
      } else {
        _this.skuQuery = "";
      }
    },
    ...
    appendLevel2QueryData(data) {
      ...
      if(_this.filterBrands&&_this.filterBrands.length){
