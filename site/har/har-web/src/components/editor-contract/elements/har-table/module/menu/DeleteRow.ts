...
class DeleteRow implements IButtonMenu {
  ...
  isDisabled(editor: IDomEditor): boolean {
    ...
    const rowNode = DomEditor.getSelectedNodeByType(editor, 'har-table-row');
    if (rowNode == null) {
  ...
  exec(editor: IDomEditor, value: string | boolean) {
    ...
    const [rowEntry] = Editor.nodes(editor as unknown as Editor, {
      match: (n) => DomEditor.checkNodeType(n, 'har-table-row'),
      universal: true
    });
