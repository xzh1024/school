...
import { DomEditor, IDomEditor, IDropPanelMenu, t } from '@wangeditor/editor';
import {
  TableCellElement,
  TableElement,
  TableRowElement
} from '../custom-types';
import { Field, TPL } from '../../../../interface';
import $, { DOMElement } from '../../../../utils/dom';

function genTableNode(
  rowNum: number,
  colNum: number,
  params?: Partial<TableElement>,
): TableElement {
  // 拼接 rows
...
class InsertTable implements IDropPanelMenu {
  ...
  private insertTable(
    ...
    if (DomEditor.isSelectedEmptyParagraph(editor)) {
      Transforms.removeNodes(editor as unknown as Editor, { mode: 'highest' });
    }

    // 插入表格
    const tpl: Array<Field> = Array.from({ length: colNum }).map(
      () => new Field()
    );
    const tableField = new Field('', '', TPL.TABLE, tpl);
    ...
    Transforms.insertNodes(editor as unknown as Editor, tableNode, { mode: 'highest' });

    editor.emit('insert-module');
  }
}
