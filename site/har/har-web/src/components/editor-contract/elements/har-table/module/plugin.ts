...
import {deleteFilter} from '@/components/editor-contract/utils/helpers';
...
function withTable<T extends IDomEditor>(editor: T): T {
  ...
  newEditor.deleteBackward = (unit) => {
    if (deleteFilter(newEditor, 'har-table', 'Backspace')) {
  ...
  newEditor.handleTab = () => {
    ...
    if (selectedNode) {
      const above = Editor.above(editor as unknown as Editor) as NodeEntry<SlateElement>;

      // 常规情况下选中文字外层 table-cell 进行跳转
      if (DomEditor.checkNodeType(above[0], 'har-table-cell')) {
        Transforms.select(editor as unknown as Editor, above[1]);
      }

      let next = Editor.next(editor as unknown as Editor);
      if (next) {
        if (next[0] && (next[0] as BaseText).text) {
          // 多个单元格同时选中按 tab 导致错位修复
          next =
            (Editor.above(editor as unknown as Editor, { at: next[1] }) as NodeEntry<Descendant>) ??
            next;
        }
        Transforms.select(editor as unknown as Editor, next[1]);
      } else {
        ...
        if (
          ...
          Transforms.insertNodes(newEditor as unknown as Editor, p, { at: [topLevelNodesLength] });
  ...
  newEditor.deleteForward = (unit) => {
    if (deleteFilter(newEditor, 'har-table', 'Delete')) {
      Message.warning('结构性数据不能删除');
      return;
    }
    const res = deleteHandler(newEditor);
  ...
  newEditor.normalizeNode = ([node, path]) => {
    ...
    if (isLast) {
      const p = DomEditor.genEmptyParagraph();
      Transforms.insertNodes(newEditor as unknown as Editor, p, { at: [path[0] + 1] });
    }
  };

  // 重写 insertData - 粘贴文本
  newEditor.insertData = (data: DataTransfer) => {
    const tableNode = DomEditor.getSelectedNodeByType(newEditor, 'har-table');
    if (tableNode == null) {
    ...
    Editor.insertText(newEditor as unknown as Editor, text);
  };
  ...
  newEditor.selectAll = () => {
    ...
    const cell = DomEditor.getSelectedNodeByType(newEditor, 'har-table-cell');
    if (cell == null) {
      selectAll();
      return;
    }
    ...
    const start = Editor.start(newEditor as unknown as Editor, path);
    const end = Editor.end(newEditor as unknown as Editor, path);
    const newSelection = {
      anchor: start,
      focus: end
    };
