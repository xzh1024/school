...
import { getFirstRowCells } from '../../../../utils/helpers';
/**
...
function getContentEditable(
  ...
  const tableStart = Editor.start(editor as unknown as Editor, tablePath);
  const tableEnd = Editor.end(editor as unknown as Editor, tablePath);
    ...
    [
      h(
        'table',
        {
          props: {
            width,
            contentEditable: editable && isTplMode
          }
        },
        [
          h(
