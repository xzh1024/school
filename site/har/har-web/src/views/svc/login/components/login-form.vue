<template>
  <div class="login-form-wrapper">
    ...
    <div class="login-form-title">储值卡管理系统</div>
...
<script lang="ts" setup>
  ...
  import {
    svcWebCaptchaImageGet,
    SvcWebCaptchaImageGetResponse
  } from '@/api/svc-web';
  ...
  // 重置token
  t.setToken('');
  const codeImg = ref<SvcWebCaptchaImageGetResponse>({
    captchaOnOff: false
  });
  ...
  const [getCodeImage] = useFuncProxy(
    async () => {
      codeImg.value = await svcWebCaptchaImageGet();
    },
    () => {
      // getCodeImage();
    }
    ...
    if (!errors) {
      ...
      try {
        ...
        await router.replace({
          name: (redirect as string) || 'svc-home',
          query: {
            ...othersQuery
          }
        });
