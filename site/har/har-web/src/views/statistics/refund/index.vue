<template>
  <div>
    ...
    <i-table
      ...
      :options="options"
      :api="cbdApiProjectReportDayReturnOrderListGet"
    ></i-table>
  </div>
</template>
...
<script setup lang="ts">
  ...
  import {
    cbdApiProjectReportDayReturnOrderChartGet,
    cbdApiProjectReportDayReturnOrderListGet
  } from '@/api';
  ...
  const [getBarData] = useFuncProxy(async () => {
    const res = await cbdApiProjectReportDayReturnOrderChartGet({
      ...search.value
    });
  ...
  const options = ref({
    ...
    columns: [
      ...
      {
        label: '退款订单金额（¥）',
        prop: 'returnOrderMoney'
      },

      {
        label: '线上退款订单数',
        prop: 'onlineReturnOrderNum'
      },
      {
        label: '线上退款订单金额（¥）',
        prop: 'onlineReturnOrderMoney'
      },
      {
        label: '线下退款订单数',
        prop: 'offlineReturnOrderNum'
      },
      {
        label: '线下退款订单金额（¥）',
        prop: 'offlineReturnOrderMoney'
      }
    ]
  });
