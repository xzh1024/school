...
<script setup lang="ts">
  ...
  import {
    groundPcApiProductAttributeUnitsAddPost,
    groundPcApiProductAttributeUnitsUpdatePost,
    groundPcApiProductAttributeUnitsInfoIdGet
  ...
  const title: Ref<string> = computed(() => {
    return props.id ? '编辑单位' : '新增单位';
  });
  const [getInfo] = useFuncProxy(async () => {
    form.value = await groundPcApiProductAttributeUnitsInfoIdGet({
      id: `${props.id}`
    });
  });
  onMounted(() => {
  ...
  async function submit(done: (closed: boolean) => void) {
    try {
      ...
      if (props.id) {
        await groundPcApiProductAttributeUnitsUpdatePost(form.value);
        Message.success('修改成功');
      } else {
        await groundPcApiProductAttributeUnitsAddPost(form.value);
        Message.success('新增成功');
      }
      await getUnitsDic(true);
      emits('confirm');
      props.refresh?.();
