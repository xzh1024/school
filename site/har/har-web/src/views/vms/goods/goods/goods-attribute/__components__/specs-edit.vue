...
<script setup lang="ts">
  ...
  import {
    groundPcApiProductAttributeSpecificationsAddPost,
    groundPcApiProductAttributeSpecificationsUpdatePost,
    groundPcApiProductAttributeSpecificationsInfoIdGet
    ...
    columns: [
      {
        ...
        prop: 'name',
        span: 24,
        rules: [{ required: true, message: '规格项 / 规格值名称' }],
        options: {
          maxLength: 50
        }
      },
  ...
  const [getInfo] = useFuncProxy(async () => {
    form.value = await groundPcApiProductAttributeSpecificationsInfoIdGet({
      id: `${props.id}`
    });
  });
  onMounted(() => {
  ...
  async function submit(done: (closed: boolean) => void) {
    try {
      ...
      if (props.id) {
        await groundPcApiProductAttributeSpecificationsUpdatePost(form.value);
        Message.success('修改成功');
      } else {
        await groundPcApiProductAttributeSpecificationsAddPost({
          ...form.value,
          pid: props.parentId
        });
