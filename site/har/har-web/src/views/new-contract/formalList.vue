<template>
  <div class="page-container">
    <i-table
      ...
      :options="options"
      :api="cbdApiProjectFormalContractListGet"
      @reset="onReset"
    >
      ...
      <template #header>
        <a-space>
          <a-button
            v-permission="['formal:contract:save', 'formal:contract:info']"
            class="i-btn"
            type="primary"
            @click="onAdd"
            >添加</a-button
          >
          <a-button
            v-permission="['formal:contract:export']"
            class="i-btn"
            type="primary"
            @click="onExport"
            >导出</a-button
          >
          <a-button
            v-permission="['formal:contract:delete']"
            class="i-btn"
            type="primary"
            @click="onHandleDel"
            >删除</a-button
          >
          <a-button
            v-permission="['formal:contract:approval:confirm']"
            class="i-btn"
            type="primary"
        ...
        <a-space>
          <div
            v-if="
              ...
              record.isModify
            "
            v-permission="['formal:contract:info', 'formal:contract:save']"
            class="s-btn"
            @click="onEdit(record)"
            >编辑</div
          >
...
<script setup lang="ts">
  ...
  import { examine_status, business_status } from '@/config/contract_status';
  ...
  const confirmExport = () => {
    const url = cbdApiProjectFormalContractExportGet.requestConfig.path;
    const params = Object.keys(search.value)
      .filter(
        (key) =>
          search.value[
            key as keyof CbdApiProjectFormalContractListGetRequest
          ] !== undefined
      )
      .reduce(
        (acc, key) => ({
          ...acc,
          [key]:
            search.value[key as keyof CbdApiProjectFormalContractListGetRequest]
        }),
        {}
      );
    if (selectedKeys.value.length > 0) {
      ...
      exportDownload(url, { ...params, contractIds: '' }, '正式合同');
    }
  };
  ...
  const confirmDel = async () => {
    console.log('确认删除');
    const d = await contractStore.delContract(contractIds.value);
    if (d) {
      contractIds.value = [];
      selectedKeys.value = [];
  ...
  const confirmExamine = async () => {
    const d = await contractStore.confirmContract(
  ...
  const search = ref<CbdApiProjectFormalContractListGetRequest>({});
