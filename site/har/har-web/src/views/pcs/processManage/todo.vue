<template>
  <div>
    <i-table
      ...
      v-permission="['web:instance:list:todo']"
      :options="page.options"
      :api="flowableEngineInstancePageTodoGet"
      @reset="tableFormReset"
    >
      <template #menu="{ record }">
        <a-button
          v-permission="['web:instance:todo:info']"
          type="text"
...
<script setup lang="ts">
  ...
  import { flowableEngineInstancePageTodoGet } from '@/api/flowable-engine';
  import { useColumns, openProcessDetail } from './hooks/useProcessTableColumn';
  const columns = useColumns('todo');
