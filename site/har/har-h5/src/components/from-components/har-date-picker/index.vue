<template>
  <view class="har-date__picker-wrap" @touchend="stop">
    <view
      ref="har_dpkm_ani"
      class="har-date__picker-mask"
      :class="{ 'har-dpk__mask-show': isShow }"
      :style="maskStyl"
    ...
    <view
      ref="har_dpk_ani"
      class="har-date__picker-content"
      :class="{
        'har-dpk__content-dark': theme === 'dark',
        'har-dpk__content-show': isShow,
        'har-date__picker-radius': radius
      }"
      :style="{ zIndex: zIndex }"
    >
      <view
        class="har-date__picker-header"
        :class="{
          'har-dpk__header-dark': theme === 'dark',
          'har-date__picker-radius': radius
        }"
        :style="headerStyl"
      >
        <text
          class="har-dpk_btn-cancel"
          :class="[
            theme === 'dark'
              ? 'har-dpk__cancel-color_dark'
              : 'har-dpk__cancel-color'
          ]"
          :style="cancelStyl"
          @tap.stop="btnCancel"
          >取消</text
        >
        <text
          class="har-date__picker-title"
          :class="[
            theme === 'dark'
              ? 'har-dpk__title-color_dark'
              : 'har-dpk__title-color'
          ]"
          :style="titleStyl"
          >{{ title }}</text
        >
        <text
          class="har-dpk__btn-sure har-dpk__sure-color"
          :style="confrimStyl"
          @tap.stop="btnConfirm"
          >确定</text
        >
      </view>
      <view v-if="range" class="har-dpk__range-wrap">
        <view
          class="har-dpk__range-inner"
          :class="{ 'har-dpk__range-dark': theme === 'dark' }"
        >
          <text
            class="har-dpk__range-text"
            :class="{ 'har-dpk__range-active': isActive == 1 }"
            :style="startStyl"
          ...
          <text
            class="har-dpk__range-text"
            :class="{ 'har-dpk__range-active': isActive == 2 }"
            :style="endStyl"
      ...
      <view @touchstart.stop="pickerstart">
        ...
        <picker-view
          ...
          :indicator-class="
            theme === 'dark' ? 'har-date__picker-indicator' : ''
          "
          class="har-date__picker-view"
        ...
        >
          <picker-view-column>
            <view
              ...
              :key="index"
              :style="contentStyl"
              class="har-date__picker-text"
              :class="{ 'har-dpk__color-dark': theme === 'dark' }"
            >
...
<script>
  ...
  export default {
    ...
    // #ifdef APP-NVUE
    mounted() {
      if (!this.$refs["har_dpkm_ani"] || !this.$refs["har_dpk_ani"]) return;
      this._transtion(
        "har_dpkm_ani",
        {
    ...
    methods: {
      ...
      getHours() {
        const range = this.hourRange || [0, 23];
      ...
      getMinutes() {
        const range = this.minuteRange || [0, 59];
        let min = Number(range[0] || 0);
      ...
      getSeconds() {
        const range = this.secondRange || [0, 59];
        let min = Number(range[0] || 0);
        let max = Number(range[1] || 59);
        min = Math.floor(min < 0 || min > 59 ? 0 : min);
        max = Math.floor(max < 0 || max > 59 ? 59 : max);
      ...
      initData() {
        ...
        this.$nextTick(() => {
          setTimeout(() => {
            const di = this.getIndex(this.days, this.values[2]);
            const hi = this.getIndex(this.hours, this.values[3]);
            const mi = this.getIndex(this.minutes, this.values[4]);
            const si = this.getIndex(this.seconds, this.values[5]);
            this.vals = [
              [index],
              [index, idx],
              [index, idx, di],
      ...
      btnConfirm(e) {
        ...
        if (this.range) {
          ...
          if (!start || !end) {
            const msg = !start ? this.start : this.end;
            uni.showToast({
              title: `请选择${msg}`,
              icon: "none"
            });
...
<style scoped>
  ...
  /* #ifndef APP-NVUE */
  .har-dpk__content-dark {
    background: #222;
  }

  /* #endif */

  .har-dpk__content-show {
    /* #ifndef APP-NVUE */
  ...
  .har-date__picker-radius {
    border-top-left-radius: 24rpx;
    border-top-right-radius: 24rpx;
    overflow: hidden;
  }

  .har-date__picker-header {
    /* #ifndef APP-NVUE */
    width: 100%;
    display: flex;
  ...
  .har-dpk__header-dark {
    background: #222;
    border-bottom-color: #333;
  }

  /* #ifndef APP-NVUE */
  .har-date__picker-header::after {
    content: "";
    position: absolute;
  ...
  .har-dpk__range-active {
    ...
    /* #ifndef APP-NVUE */
    background: var(--har-color-primary, #465cff);
    /* #endif */
  }
  ...
  .har-dpk_btn-cancel:active {
    opacity: 0.5;
  }

  .har-dpk__btn-sure:active {
    opacity: 0.5;
  }

  .har-date__picker-title {
    font-weight: normal;
    font-size: 28rpx;
    flex: 1;
    padding: 0 24rpx;
  ...
  /* #ifndef APP-NVUE*/
  .har-date__picker-indicator::v-deep {
    position: relative;
    border-color: transparent;
  }

  .har-date__picker-indicator::v-deep::before {
    border-color: #333;
  }

  .har-date__picker-indicator::v-deep::after {
    border-color: #333;
  }
